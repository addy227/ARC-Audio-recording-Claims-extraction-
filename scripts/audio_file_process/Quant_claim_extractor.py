...
MODEL_NAME = llm.get('model_name', 'llama3.1-finetuned') if 'finetuned_model_path' in llm else (
    llm.get('model_name', 'llama3') if 'model_name' in llm else "llama3")
...
def log_gpu_memory(context: str, device: str = "cuda"):
    ...
    if mem_info:
        logger.info(
            f"💾 [Quantized Finetuned Model] GPU Memory [{context}]: "
        ...
        logger.debug(f"💾 [Quantized Finetuned Model] GPU Memory [{context}]: Not available (CPU mode)")
...
try:
    ...
    # Note: When using quantization with load_in_4bit=True, the model is automatically
    # placed on CUDA if available, even without device_map="auto"
    # We avoid using device_map="auto" to avoid requiring accelerate library
    ...
    # With quantization, the model should already be on CUDA if available
    # But we can explicitly move it if needed (though usually not necessary)
    if DEVICE == "cpu":
        # Only explicitly move to CPU if that's the target device
        model = model.to(DEVICE)
        logger.info("✅ Model loaded with 4-bit quantization on CPU")
    else:
        # For CUDA, quantization automatically places model on GPU
        logger.info("✅ Model loaded with 4-bit quantization on CUDA")

    log_gpu_memory("After Model Load", DEVICE)

    model.eval()  # Set to evaluation mode
    logger.info(f"✅ Finetuned Llama3.1 8B instruct model loaded successfully with 4-bit quantization on {DEVICE.upper()}")

except Exception as e:
    logger.error(f"❌ Failed to load finetuned model with quantization: {e}", exc_info=True)
    logger.warning("Attempting to load model without quantization as fallback...")
    try:
        ...
        model = model.to(DEVICE)
        model.eval()
        logger.warning("⚠️ Loaded model without quantization (fallback mode)")
    except Exception as fallback_error:
        logger.error(f"❌ Fallback loading also failed: {fallback_error}", exc_info=True)
...
PROMPT_TEMPLATE = """You are a data extraction assistant. Your task is to extract **structured medical patient-related claim information** from a given conversation transcript and - Strictly do not extract provider-only information.
...
- patient_first_name and patient_last_name: Extract exactly as spoken in context, do not assume or guess names. More information regarding names can be written in the form of letters for example "last name is spelled as S as in Sarah, A as in alpha, N as in November, E as in echo, V as in Victor, O as in Oscar, R as in Romeo, E as in Alpha, V as in Victor, O as in Oscar, N as in November, and G as in golf" which comes out to  "SANEVOREVONG" By compiling the letters mentioned
- Dates: Use ISO format `MM-DD-YYYY`.
- Amounts: Extract exactly as spoken.
- Names: Extract exactly as spoken. Do not Merge 2 names together at any cost, for example if full name is "Helen Carol Jones", patient_first_name is "Helen" and patient_last_name is "Carol Jones", Do NOT save patient_last_name as "Caroljones"
                    ...
                    if bracket_count == 0:
                        ...
                        try:
                            ...
                            if isinstance(parsed, list):
                                logger.debug(
                                    "Successfully extracted JSON from Python dict syntax using ast.literal_eval")
                                return parsed
                        except (ValueError, SyntaxError) as e:
...
def call_finetuned_model(prompt: str) -> str:
    """Call finetuned Llama3.1 8B instruct model with proper chat template formatting.

    Generates response using the loaded finetuned model. Properly formats the prompt
    using Llama3.2 instruct chat template and handles special tokens correctly.
    ...
    Notes:
        Exceptions are caught and logged internally; callers receive a fallback.
    """
    if model is None:
        raise RetryableError("Model not loaded", error_code="MODEL_NOT_LOADED")

    try:
        logger.info(f"Model name {MODEL_NAME} (Llama3.1 8B Instruct - 4-bit Quantized)")
        log_gpu_memory("Before Inference (Quantized)", DEVICE)

        # Apply Llama3.2 instruct chat template
        if hasattr(tokenizer, 'apply_chat_template') and tokenizer.chat_template is not None:
            # Format as a user message for Llama3.2 instruct format
            ...
            logger.debug("Applied Llama3.2 instruct chat template to prompt")
            logger.debug(f"Formatted prompt preview (first 300 chars): {formatted_prompt[:300]}")
        else:
            # Fallback: manually format for Llama3.2 if template not available
            logger.warning("Chat template not found, using manual Llama3.2 format")
            formatted_prompt = f"<|start_header_id|>user<|end_header_id|>\n\n{prompt}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"

        # Tokenize the formatted prompt
        inputs = tokenizer(formatted_prompt, return_tensors="pt", truncation=True, max_length=LLAMA3_8B_CONTEXT_LIMIT)
        ...
        log_gpu_memory("After Inference (Quantized)", DEVICE)
        ...
        # Remove Llama3.2 special tokens from response
        # These tokens might appear in the response: <|eot_id|>, <|start_header_id|>, etc.
        ...
        # Additional cleaning: Remove any incomplete/malformed content at the end
        # The model might output incomplete JSON, so try to find the last complete structure
        # Look for the last complete closing bracket
        if response.count('[') > response.count(']'):
            # Unbalanced brackets - try to fix by finding last complete structure
            last_complete_bracket = response.rfind(']')
            if last_complete_bracket != -1:
                # Check if there's a complete JSON structure before this
                potential_json = response[:last_complete_bracket + 1]
                ...
                for i in range(len(potential_json) - 1, -1, -1):
                    if potential_json[i] == ']':
                        ...
                        bracket_count -= 1
                        if bracket_count == 0:
                            # Found matching bracket, use this as the response
                            response = potential_json[i:]
                            logger.debug(f"Trimmed incomplete response, using: {response[:200]}...")
                            break
        ...
        if total_tokens > LLAMA3_8B_CONTEXT_LIMIT:
            ...
            logger.info("Token count within model context window.")
...
def extract_claims_from_chunk(chunk: str, filename: str | None = None) -> list[tuple[str | None, dict]]:
    ...
    output = call_finetuned_model(prompt)
    ...
    try:
        ...
        if not parsed_data:
            ...
            if filename:
                debug_file = Path(
                    LOG_DIR) / f"failed_extraction_{filename}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                try:
    ...
    for idx, group in enumerate(parsed_data):
        ...
        claim_field_names = ['patient_first_name', 'patient_last_name', 'member_id', 'npi_id',
                             'date_of_birth', 'date_of_service', 'billed_amount']
...
def filter_and_deduplicate_claims(claim_list: List[Tuple[str | None, dict]]) -> List[Dict]:
    ...
    Note:
        Empty claims are defined as having null names, zero billed amount, and null dates.
        Claims are deduplicated based on provider tax ID and claim content.
    ...
    for tax_id, claim in claim_list:
        ...
        # Skip empty claims
        if (
                (claim.get("patient_first_name") is None)
                and (claim.get("patient_last_name") is None)
...
def save_individual_claims(
        ...
        if audio_path.exists():
            try:
                ...
                if audio_file_blob_id:
                    ...
                    logger.warning(
                        f"⚠️ Audio file upload returned None (upload may have failed - check logs above): {audio_path}")
            except Exception as e:
        ...
        if transcript_path.exists():
            try:
                ...
                if transcript_file_blob_id:
                    ...
                    logger.warning(
                        f"⚠️ Transcript file upload returned None (upload may have failed - check logs above): {transcript_path}")
            except Exception as e:
    ...
    for group in claims_data:
        ...
        for claim in claims:
            ...
            output_filename = f"{safe_provider}_{safe_first}_{safe_last}_{safe_member}_{timestamp_root}.json"
            ...
            try:
                ...
                if claim_json_file_id:
                    ...
                    structured_output["ARRecordingDetails"]["ClaimsList"][0][
                        "claim_json_attributes_storage_id"] = claim_json_file_id
                    ...
                    logger.warning(
                        f"⚠️ Claim JSON upload failed, but claim data will still be returned with audio/transcript blob IDs")
...
def process_all_transcripts(files):
    ...
    logger.info("📄 Stage 3: Claim Extraction started (Finetuned Model - Quantized)")
    start_time = time.time()
