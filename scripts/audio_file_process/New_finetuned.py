...
MODEL_DIR = "/home/nalabotalaadvait/Documents/Llama3.1_Finetuned_March28"
...
- Alphanumerical Value: Use NATO phonetic alphabet to spell alphabets accurately. More information regarding alphabets can be written in the form of phonetic alphabets.
---
...
- **ID Formats:**
  ...
  - `"member_id"`: an Alphanumerical Identifier consisting of numbers and/or letters that may include letters spelled out as words; Use NATO phonetic alphabet to spell alphabets accuretly. * "HSN Hotel" = H , "HOTEL" = H *
...
- Member ID: Extract alphabets using NATO phonetic alphabet and combine with numbers.
...
[
  {{
    ...
    "claims": [
      {{
        "provider_npi_id": "string or null",
...
def save_individual_claims(
    ...
    for group in claims_data:
        ...
        for claim in claims:
            ...
            logger.info(f"🔍 Claim keys after sanitize: {list(claim.keys())}")
            logger.info(f"🔍 Summary value after sanitize: {claim.get('summary')}")
