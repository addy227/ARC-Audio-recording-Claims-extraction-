# """
# VoiclaimBot Claim Extraction Script
#
# Extracts structured healthcare claim data from cleaned speech-to-text (STT) transcript files using a locally running LLM (Ollama).
#
# Features:
...
# from utils.until_master import (
#     get_project_path,
#     sanitize_claim,
...
# EXTRACTED_CLAIMS_DIR = "/CosmosAI/voicedata/local_data_source/extracted_claims/"
# CLEANED_AUDIO_DIR = "/CosmosAI/voicedata/local_data_source/cleaned_audio/"
# RAW_AUDIO_DIR ="/CosmosAI/voicedata/PBX_Call_Recordings/"
#
# MODEL_NAME = llm['model_name'] if 'model_name' in llm else "llama3"
#
...
#
# FALLBACK_TAX_ID = audio_cleaner_cfg.get("fallback_tax_id", "NA")
# FALLBACK_FIRSTNAME = audio_cleaner_cfg.get("fallback_firstname", "NA")
# FALLBACK_LASTNAME = audio_cleaner_cfg.get("fallback_lastname", "NA")
# FALLBACK_MEMBER_ID = audio_cleaner_cfg.get("fallback_member_id", "NA")
...
#   - `"npi_id"`: exactly 10-digit numeric string, typically introduced with terms like "NPI" or "provider ID".
#   - `"member_id"`: an identifier consisting of numbers and/or letters that may include letters spelled out as words; convert any spelled-out letters to their corresponding single-character alphabetic form and present the result in a clear, standardized format. Do NOT extract or include invalid IDs or misclassify similar-looking numbers.
# - If any ambiguity arises, assign `null` rather than guessing.
#
# ### Field Pattern Hints:
# - Tax ID: 9-digit numeric, no letters.
# - NPI ID: 10-digit numeric.
# - Member ID: an identifier consisting of numbers and/or letters.
# - patient_first_name and patient_last_name: Extract exactly as spoken in context, do not assume or guess names.
# - Dates: Use ISO format `MM-DD-YYYY`.
# - Amounts: Extract exactly as spoken.
# - Names: Extract exactly as spoken.
#
# ---
#
...
# - patient_first_name
# - patient_last_name
# - date_of_dob (format: MM-DD-YYYY or null)
# - billed_amount
...
#         "date_of_dob": "MM-DD-YYYY or null",
...
# """
#
# class Claim(BaseModel, extra=Extra.allow):
#     """
...
#     """Call Ollama model with streaming and basic token accounting.
#
#     Streams response chunks and concatenates into a single string. Logs token usage
#     and warns if token count exceeds the model context limit.
#
#     Args:
#         prompt: Prompt string to send to the model.
#
#     Returns:
#         Model response as a string. Returns '[]' on exception.
#
#     Notes:
...
#         raise RetryableError(f"LLM processing failed: {e}", error_code="LLM_ERROR", details={"chunk_length": len(chunk)})
#
#
#
#
...
#         tax_id = group.get("tax_id") if isinstance(group.get("tax_id"), str) else None
...
#
#     Note:
#         Empty claims are defined as having UNKNOWN names, zero billed amount, and null dates.
...
#         # Skip empty claims
#         if (
#             claim.get("patient_first_name") == "UNKNOWN"
#             and claim.get("patient_last_name") == "UNKNOWN"
#             and claim.get("billed_amount") == "0.00"
#             and claim.get("date_of_service") is None
#             and claim.get("date_of_dob") is None
#         ):
#             continue
#
...
#             # Normalize provider tax ID if needed
#             if tax_id:
#                 tax_id = str(tax_id).strip()
#
#             provider_tax_id = tax_id
#             firstname = claim.get("patient_first_name", FALLBACK_FIRSTNAME)
#             lastname = claim.get("patient_last_name", FALLBACK_LASTNAME)
#             member_id = claim.get("member_id", FALLBACK_MEMBER_ID)
#
#             # Ensure filesystem-safe components
#             safe_provider = safe_filename(str(provider_tax_id))
#             safe_first = safe_filename(str(firstname))
#             safe_last = safe_filename(str(lastname))
#             safe_member = safe_filename(str(member_id))
#
...
#                             "member_id": member_id,
#                             "provider_npi_id": claim.get("npi_id"),
#                             "date_of_service": claim.get("date_of_service"),
#                             "date_of_dob": claim.get("date_of_dob"),
...
#
#             try:
#                 # Save the structured claim JSON to disk
#                 with open(output_path, "w", encoding="utf-8") as out_file:
#                     json.dump(structured_output, out_file, indent=2)
#
#                 # Upload the claim JSON file to blob storage and get its ID
#                 claim_json_file_id = upload_file_to_blob(output_path)
#
#                 if claim_json_file_id:
#                     # Update the storage ID in the JSON and re-save
...
#
#     logger.info("📄 Stage 3: Claim Extraction started")
#     start_time = time.time()
#
...
#     """
#     transcripts_dir = '/CosmosAI/voicedata/local_data_source/transcripts/testing_transcript.txt"'
#
...
#         print(f"getting transcript files: {transcripts_dir}")
