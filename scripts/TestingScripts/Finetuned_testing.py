...
MODEL_DIR = "/home/nalabotalaadvait/Documents/Llama3.1_April01-V2"
...
- **ID Formats:**
  ...
  - `"member_id"`: an Alphanumerical Identifier consisting of numbers and/or letters that may include letters spelled out as words; Use NATO phonetic alphabet to spell alphabets accuretly.
...
def call_finetuned_model(transcript_text: str) -> str:
    ...
    try:
        ...
        # Fix unbalanced brackets if model truncated output
        if response.count('[') > response.count(']'):
            last_bracket = response.rfind(']')
            if last_bracket != -1:
                potential = response[:last_bracket + 1]
                bracket_count = 0
                for i in range(len(potential) - 1, -1, -1):
                    if potential[i] == ']':
                        ...
                        bracket_count -= 1
                        if bracket_count == 0:
                            response = potential[i:]
                            logger.debug(f"Trimmed incomplete response: {response[:200]}...")
                            break
